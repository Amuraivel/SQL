...
— (PS: I believe the union_innopanel_patentfamilies table is broken because it doesn’t full join our tables before inner joining them on the full docdb table, but we subset anyway so it doesn’t matter....)
...
SELECT DISTINCT fam.docdb_family_id,fam.inpadoc_family_id, CONCAT(eee."HRM_L2") AS harmonizedName,
...
— VIew some data
SELECT docdb_family_id, inpadoc_family_id, florianuba, markuba, harmonizedname,personname,florianpanelname,markpanelname
 FROM complement_innopanel_patentfamilies
  LIMIT 1000